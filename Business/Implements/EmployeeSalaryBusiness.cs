...
namespace Business.Implements
{
    public class EmployeeSalaryBusiness : IEmployeeSalaryBusiness
    {
        ...
        public void DeleteByIDEmployee(long IDEmployee)
        {
            var salarys = _employeeSalaryRepository.SelectAll();
            foreach(var item in salarys.Where(x => x.IDEmployee == IDEmployee))
            {
                _employeeSalaryRepository.DeleteByItem(item);
